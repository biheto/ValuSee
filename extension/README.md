...
After updating the unpacked extension, click **Reload** on the browser extension management page. Version 0.4.3 uses the V6 collector protocol so an already-open tab cannot silently reuse the previous collector. It validates saved sessions against the API, reports missing title, price, or SKU fields separately, and displays the active collector version beside the source. It ranks title candidates to reject review, sales and navigation labels, prioritizes the selected JD SKU and labeled JD price state, prefers visible after-coupon/effective prices on Taobao and Tmall, records the original price without double-counting discounts, and normalizes selected variants and store names. Pinduoduo wholesale pages retain the full visible title, minimum and maximum wholesale prices, minimum order quantity and available package variants; the editable price is explicitly labeled as the range minimum until the user confirms a package and quantity.
