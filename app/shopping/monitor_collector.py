...
def collect_public_monitor_updates(
    ...
    *,
    ...
    counts = {"public_checked": 0, "pending_confirmations": 0, "recapture_reminders": 0}
    for monitor in store.list_monitors():
        if monitor["status"] != "watching":
            continue
        url = str(monitor["product"].get("url") or "")
        if not platform_for_url(url) or not _poll_due(store.get_monitor_poll(monitor["monitor_id"]), monitor.get("frequency", "daily"), current_time):
            continue
        ...
        store.record_monitor_poll(
            monitor["monitor_id"], checked_at=current_time.isoformat(), status=status,
            observed_price=price, message=str(result.get("notes") or status),
        )
        if status in {"blocked", "unavailable"} or price <= 0:
            created = store.create_notification(
                user_id=monitor["user_id"], kind="recapture_required",
        ...
        candidate = {
            **monitor["product"],
            **{key: value for key, value in result.items() if key not in {"fetch_status", "cached", "fetch_error"}},
        ...
        store.create_extension_capture(
            user_id=monitor["user_id"], product=candidate,
            source="public_monitor_refresh", captured_at=current_time.isoformat(),
        )
        hint = "可能已达到目标价" if price <= float(monitor["target_price"]) else "公开价格发生变化"
        store.create_notification(
            user_id=monitor["user_id"], kind="price_confirmation_required",
...
def _poll_due(last_poll: dict[str, Any] | None, frequency: str, now: datetime) -> bool:
    if not last_poll:
        return True
    try:
        checked_at = datetime.fromisoformat(str(last_poll["checked_at"]).replace("Z", "+00:00"))
