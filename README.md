...
ValuSee 已内置拼多多多多进宝官方接口适配器。应用审核和接口权限通过后，在服务端配置 `PDD_CLIENT_ID`、`PDD_CLIENT_SECRET` 即可启用首页真实商品搜索；创建推广位后再配置 `PDD_PID`，系统会生成明确披露的推广链接。搜索结果可以直接加入候选清单，继续进行 SKU 核对、到手价计算和多 Agent 决策。

`client_secret` 只保存在服务端部署环境，禁止写入前端、提交到 Git 或通过截图公开。未配置完整凭证时，拼多多来源不会启动，也不会回退到演示商品。
