...
ValuSee 已内置拼多多多多进宝官方接口适配器。当前官方商品搜索要求应用凭证和推广身份均已通过授权备案。服务端必须同时配置 `PDD_CLIENT_ID`、`PDD_CLIENT_SECRET`、`PDD_PID` 和备案时使用的 `PDD_CUSTOM_PARAMETERS`，四项齐全后才会启用首页真实商品搜索和明确披露的推广链接。搜索结果可以直接加入候选清单，继续进行 SKU 核对、到手价计算和多 Agent 决策。

`client_secret` 只保存在服务端部署环境，禁止写入前端、提交到 Git 或通过截图公开。`PDD_PID` 与 `PDD_CUSTOM_PARAMETERS` 必须和多多进宝备案授权时提交的参数完全一致；缺少或不一致会返回错误 `60001`。未配置完整凭证时，拼多多来源不会启动，也不会回退到演示商品。
