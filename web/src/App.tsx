...
export function App() {
  ...
  const refreshRecords = async () => {
    try { const [m, p] = await Promise.all([request<Monitor[]>('/api/v1/shopping/monitors?user_id=local-user'), request<Purchase[]>('/api/v1/shopping/purchases?user_id=local-user')]); setMonitors(m); setPurchases(p); } catch { /* backend may be offline while the static preview is open */ }
  };
  ...
  return <main className="valuesee-app">
    <header className="app-header"><div className="brand-lockup"><div className="brand-mark">V</div><div><strong>ValuSee</strong><span>买之前，先看清价值</span></div></div><nav>{nav.map(([key, label, Icon]) => <button className={view === key ? 'active' : ''} key={key} onClick={() => setView(key)}><Icon size={17} />{label}</button>)}</nav><button className="profile-button">本地账户</button></header>
    {message && <div className="toast"><CheckCircle2 size={16} />{message}<button onClick={() => setMessage('')}>关闭</button></div>}
    {error && <div className="error-banner">{error}</div>}
    {view === 'analyze' && <>
      <section className="hero"><div className="hero-copy"><div className="eyebrow"><Sparkles size={16} />AI 购物决策助手</div><h1>别只看便不便宜，先看它值不值得。</h1><p>识别真假同款，算清真实到手价，结合你的预算和设备给出建议。买完之后，继续帮你盯住降价与保价。</p><form className="decision-box" onSubmit={runDecision}><textarea value={goal} onChange={(e) => setGoal(e.target.value)} /><div className="decision-controls"><label>预算 <input type="number" min={0} value={budget} onChange={(e) => setBudget(Number(e.target.value))} /> 元</label><button type="submit" disabled={loading}>{loading ? <Loader2 className="spin" size={18} /> : <Search size={18} />}立即分析</button></div></form></div><div className="hero-aside"><div className="hero-orbit"><div className="hero-orbit-core">V</div><span className="orbit-chip chip-one">同款识别</span><span className="orbit-chip chip-two">到手价</span><span className="orbit-chip chip-three">买后提醒</span></div></div></section>
