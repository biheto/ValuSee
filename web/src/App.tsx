...
function SavedWork({ reports, comparisons, onOpenReport, onOpenComparison, onDeleteComparison }: { reports: SavedReport[]; comparisons: SavedComparison[]; onOpenReport: (report: SavedReport) => void; onOpenComparison: (comparison: SavedComparison) => void; onDeleteComparison: (id: string) => Promise<void> }) {
  return <section className="page-section"><PageTitle icon={<History size={22} />} title="报告与清单" subtitle="报告和候选清单保存在你的账户中，可在不同设备继续。" /><div className="saved-work-grid"><section className="panel list-panel"><h3>购买决策报告 <span>{reports.length}</span></h3>{reports.length ? reports.map((report) => <article className="history-card" key={report.report_id}><div><strong>{report.result.summary || report.goal}</strong><span>{date(report.created_at)} · {report.products.length} 个候选</span></div><button className="soft-button" onClick={() => onOpenReport(report)}>打开 <ChevronRight size={16} /></button></article>) : <Empty text="完成分析后，报告会安全保存在这里" />}</section><section className="panel list-panel"><h3>已保存清单 <span>{comparisons.length}</span></h3>{comparisons.length ? comparisons.map((comparison) => <article className="history-card" key={comparison.comparison_id}><div><strong>{comparison.name}</strong><span>{date(comparison.updated_at)} · {comparison.products.length} 个候选</span></div><div className="row-actions"><button className="soft-button" onClick={() => onOpenComparison(comparison)}>继续比较</button><button className="danger icon-button" title="删除清单" onClick={() => void onDeleteComparison(comparison.comparison_id)}><Trash2 size={15} /></button></div></article>) : <Empty text="保存候选商品后，可稍后继续比较" />}</section></div></section>;
}
