...
export function App() {
  ...
  const refreshRecords = async () => {
    try { const [m, p, c, n] = await Promise.all([request<Monitor[]>('/api/v1/shopping/monitors'), request<Purchase[]>('/api/v1/shopping/purchases'), request<Capture[]>('/api/v1/shopping/extension/captures'), request<Notification[]>('/api/v1/shopping/notifications')]); setMonitors(m); setPurchases(p); setCaptures(c.filter((item) => item.status === 'pending_confirmation')); setNotifications(n); } catch { /* backend may be offline while the static preview is open */ }
  };
