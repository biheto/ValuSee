...
type SupportTicket = { ticket_id: string; purchase_id?: string; category: string; subject: string; status: string; updated_at: string; messages?: Array<{ message_id: string; actor_role: string; content: string; created_at: string }> };
...
export function App() {
  ...
  useEffect(() => { const syncRoute = () => setDetailRef(window.location.pathname.match(/^\/product\/(prd_[a-f0-9]+)$/)?.[1] || ''); window.addEventListener('popstate', syncRoute); return () => window.removeEventListener('popstate', syncRoute); }, []);
  useEffect(() => {
  ...
  async function addProduct(product: Product, openDetail = false) { setProducts((items) => items.some((item) => item.url && item.url === product.url) ? items : [...items, product]); if (openDetail) setDetailProduct(product); try { const registered = await request<{ product_ref: string }>('/api/v1/shopping/products', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(product) }); if (product.url) await request('/api/v1/shopping/saved', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ item_type: 'recent', reference_key: product.url, label: product.title, product }) }); if (openDetail) { setDetailRef(registered.product_ref); window.history.pushState({}, '', `/product/${registered.product_ref}`); } } catch { if (openDetail) setError('商品详情暂时无法持久化，请检查商品链接或型号。'); } await refreshRecords(); }
  ...
  return <main className={`valuesee-app experiment-${navigationVariant}`} id="main-content"><a className="skip-link" href="#primary-view">跳到主要内容</a>
    ...
    {view === 'profile' && <AccountProfilePanel onName={(name) => { setAccountName(name); localStorage.setItem('valuesee-account-name', name); }} />}
    {view === 'analyze' && <>
    ...
    {(detailRef || detailProduct) && <ProductDetail productRef={detailRef || undefined} fallbackProduct={detailProduct} favorite={Boolean(detailProduct && savedItems.some((item) => item.item_type === 'favorite' && item.reference_key === (detailProduct.url || `${detailProduct.brand}:${detailProduct.model}:${detailProduct.sku}`)))} onProductLoaded={setDetailProduct} onClose={() => { setDetailProduct(null); setDetailRef(''); window.history.pushState({}, '', '/'); }} onFavorite={(product) => void toggleFavorite(product)} onCompare={(product) => { void addProduct(product); setDetailProduct(null); setDetailRef(''); window.history.pushState({}, '', '/'); setView('analyze'); }} request={request} />}
...
function ComparisonWorkbench({ products, onChange, onShare }: { products: Product[]; onChange: (products: Product[]) => void; onShare: () => void }) {
  ...
  function move(from: number, to: number) { if (from === to || to < 0 || to >= products.length) return; const next = [...products]; const [item] = next.splice(from, 1); next.splice(to, 0, item); onChange(next); }
  function exportPng() {
    const width = 1400, rowHeight = 58, height = 190 + products.length * rowHeight;
    const canvas = document.createElement('canvas'); canvas.width = width; canvas.height = height;
    const context = canvas.getContext('2d'); if (!context) return;
    context.fillStyle = '#f7faf7'; context.fillRect(0, 0, width, height);
    context.fillStyle = '#243832'; context.font = '700 38px system-ui'; context.fillText('ValuSee 商品对比', 64, 62);
    context.fillStyle = '#71817a'; context.font = '20px system-ui'; context.fillText(`生成于 ${new Date().toLocaleString('zh-CN')} · 价格与优惠请在下单前回到来源核验`, 64, 100);
    products.forEach((product, index) => { const y = 155 + index * rowHeight; context.fillStyle = index % 2 ? '#eef5f0' : '#ffffff'; context.fillRect(48, y - 33, width - 96, 48); context.fillStyle = '#344841'; context.font = '600 21px system-ui'; context.fillText(`${index + 1}. ${product.title}`.slice(0, 55), 68, y); context.fillStyle = '#d85d68'; context.fillText(money(finalPrice(product)), 1030, y); context.fillStyle = '#64756e'; context.fillText(product.platform || '来源待确认', 1190, y); });
    const anchor = document.createElement('a'); anchor.download = `ValuSee-compare-${Date.now()}.png`; anchor.href = canvas.toDataURL('image/png'); anchor.click();
  }
  const matrixColumns = { gridTemplateColumns: `140px repeat(${products.length}, minmax(130px, 1fr))` };
  return <section className="comparison-workbench panel" aria-label="对比工作台"><div className="comparison-workbench-head"><div><span>对比工作台</span><h2>调整顺序，聚焦真正差异</h2></div><div className="comparison-tools"><button className={onlyDifferences ? 'active' : ''} onClick={() => setOnlyDifferences((value) => !value)}><ListFilter size={15} />{onlyDifferences ? '显示全部' : '仅看差异'}</button><button title="分享对比" onClick={onShare}><Share2 size={15} />分享</button><button title="导出 PNG" onClick={exportPng}><ImageDown size={15} />图片</button><button title="打印或保存 PDF" onClick={() => window.print()}><Printer size={15} />PDF</button></div></div><div className="comparison-order">{products.map((product, index) => <article key={`${product.url}-${index}`} draggable onDragStart={() => setDragIndex(index)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (dragIndex !== null) move(dragIndex, index); setDragIndex(null); }}><GripVertical size={17} /><div><strong>{index + 1}. {product.title}</strong><span>{product.platform || '来源待确认'} · {money(finalPrice(product))}</span></div><div><button title="前移" disabled={index === 0} onClick={() => move(index, index - 1)}>↑</button><button title="后移" disabled={index === products.length - 1} onClick={() => move(index, index + 1)}>↓</button></div></article>)}</div><details className="dimension-matrix" open><summary>规格维度 {onlyDifferences ? `· ${visibleDimensions.length} 项差异` : `· ${dimensions.length} 项`}</summary>{visibleDimensions.length ? <div className="dimension-table"><div className="dimension-row dimension-head" style={matrixColumns}><b>规格</b>{products.map((product, index) => <span key={index}>候选 {index + 1}</span>)}</div>{visibleDimensions.map((dimension) => <div className="dimension-row" style={matrixColumns} key={dimension}><b>{dimension}</b>{products.map((product, index) => <span key={index}>{product.specs?.[dimension] || '未提供'}</span>)}</div>)}</div> : <p>当前候选没有可比较的规格差异，请补充结构化规格。</p>}</details></section>;
}
...
function AccountProfilePanel({ onName }: { onName: (name: string) => void }) {
  type Profile = { display_name: string; email: string; email_verified: boolean; bio: string; locale: string; currency: string; avatar_url?: string | null };
  const [profile, setProfile] = useState<Profile | null>(null), [bindings, setBindings] = useState<Array<{ provider: string; account_hint: string; status: string }>>([]), [audits, setAudits] = useState<Array<{ audit_id: string; action: string; created_at: string }>>([]), [avatar, setAvatar] = useState(''), [notice, setNotice] = useState('');
  async function load() { const [next, linked, history] = await Promise.all([request<Profile>('/api/v1/auth/profile'), request<{ bindings: typeof bindings }>('/api/v1/auth/bindings'), request<{ items: typeof audits }>('/api/v1/auth/audits')]); setProfile(next); setBindings(linked.bindings); setAudits(history.items); if (next.avatar_url) { const response = await fetch(next.avatar_url, { headers: { Authorization: `Bearer ${localStorage.getItem('valuesee-token') || ''}` } }); if (response.ok) { const url = URL.createObjectURL(await response.blob()); setAvatar((old) => { if (old) URL.revokeObjectURL(old); return url; }); } } }
  useEffect(() => { void load().catch(() => setNotice('个人资料读取失败。')); return () => { if (avatar) URL.revokeObjectURL(avatar); }; }, []);
  async function save(event: FormEvent) { event.preventDefault(); if (!profile) return; const next = await request<Profile>('/api/v1/auth/profile', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(profile) }); setProfile(next); onName(next.display_name); setNotice('个人资料已保存。'); await load(); }
  async function upload(file: File) { const form = new FormData(); form.append('file', file); await request('/api/v1/auth/profile/avatar', { method: 'POST', body: form }); setNotice('头像已更新。'); await load(); }
  if (!profile) return <section className="page-section"><PageTitle icon={<UserRound size={22} />} title="个人资料" subtitle="管理公开昵称与本地化偏好。" /><div className="panel"><Empty text={notice || '正在读取资料'} /></div></section>;
  return <section className="page-section"><PageTitle icon={<UserRound size={22} />} title="个人资料" subtitle="管理头像、昵称、本地化偏好和真实账户绑定。" />{notice && <div className="toast"><CheckCircle2 size={16} />{notice}</div>}<div className="profile-settings-grid"><form className="panel compact-form" onSubmit={save}><div className="avatar-editor">{avatar ? <img src={avatar} alt="当前头像" /> : <UserRound size={32} />}<label><Camera size={15} />更换头像<input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => event.target.files?.[0] && void upload(event.target.files[0])} /></label></div><label>显示名称<input required maxLength={60} value={profile.display_name} onChange={(event) => setProfile({ ...profile, display_name: event.target.value })} /></label><label>个人说明<textarea maxLength={300} value={profile.bio} onChange={(event) => setProfile({ ...profile, bio: event.target.value })} /></label><label>界面语言<select value={profile.locale} onChange={(event) => setProfile({ ...profile, locale: event.target.value })}><option value="zh-CN">简体中文</option><option value="en-US">English</option></select></label><label>默认币种<select value={profile.currency} onChange={(event) => setProfile({ ...profile, currency: event.target.value })}>{['CNY','USD','EUR','JPY'].map((item) => <option key={item}>{item}</option>)}</select></label><button className="primary-button"><Save size={15} />保存资料</button></form><div className="profile-settings-side"><section className="panel"><h2>账户绑定</h2>{bindings.map((item) => <article className="binding-row" key={item.provider}><div><strong>{item.provider}</strong><span>{item.account_hint}</span></div><b className={item.status}>{item.status === 'verified' ? '已验证' : '待验证'}</b></article>)}</section><section className="panel"><h2>操作日志</h2>{audits.length ? audits.slice(0, 20).map((item) => <article className="audit-row" key={item.audit_id}><strong>{item.action === 'profile.updated' ? '更新个人资料' : item.action === 'avatar.updated' ? '更新头像' : item.action}</strong><span>{date(item.created_at)}</span></article>) : <Empty text="还没有资料操作记录" />}</section></div></div></section>;
}
...
function FamilyWorkspacePanel() {
  type Family = { family_id: string; name: string; owner_id: string; role: string };
  type Member = { user_id: string; display_name: string; email: string; role: string };
  type Invitation = { invitation_id: string; family_name: string; role: string; expires_at: string };
  type Asset = { asset_id: string; name: string; category: string; brand?: string; model?: string; warranty_deadline?: string };
  const [families, setFamilies] = useState<Family[]>([]), [members, setMembers] = useState<Member[]>([]), [invitations, setInvitations] = useState<Invitation[]>([]), [assets, setAssets] = useState<Asset[]>([]);
  const [selected, setSelected] = useState(''), [name, setName] = useState('我的家庭'), [inviteEmail, setInviteEmail] = useState(''), [notice, setNotice] = useState('');
  const [assetName, setAssetName] = useState(''), [assetCategory, setAssetCategory] = useState('数码设备');
  const [budget, setBudget] = useState({ monthly_budget: 0, annual_budget: 0, currency: 'CNY' });
  const current = families.find((item) => item.family_id === selected);
  async function loadFamily(id: string) { if (!id) return; const [nextMembers, nextAssets, nextBudget] = await Promise.all([request<Member[]>(`/api/v1/families/${encodeURIComponent(id)}/members`), request<Asset[]>(`/api/v1/families/${encodeURIComponent(id)}/assets`), request<typeof budget>(`/api/v1/families/${encodeURIComponent(id)}/budget`)]); setMembers(nextMembers); setAssets(nextAssets); setBudget(nextBudget); }
  async function load() { if (!localStorage.getItem('valuesee-token')) return; const [rows, pending] = await Promise.all([request<Family[]>('/api/v1/families'), request<Invitation[]>('/api/v1/families/invitations/pending')]); setFamilies(rows); setInvitations(pending); const id = selected || rows[0]?.family_id || ''; setSelected(id); await loadFamily(id); }
  useEffect(() => { void load().catch(() => setNotice('家庭数据读取失败。')); }, []);
  async function create(event: FormEvent) { event.preventDefault(); await request('/api/v1/families', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name }) }); setNotice('家庭已创建。'); await load(); }
  async function invite(event: FormEvent) { event.preventDefault(); await request('/api/v1/families/invite', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ family_id: selected, email: inviteEmail }) }); setInviteEmail(''); setNotice('邀请已发送，等待对方确认。'); await load(); }
  async function respond(id: string, accept: boolean) { await request(`/api/v1/families/invitations/${encodeURIComponent(id)}/respond`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ accept }) }); setNotice(accept ? '已加入家庭。' : '已拒绝邀请。'); await load(); }
  async function role(userId: string, nextRole: string) { await request(`/api/v1/families/${encodeURIComponent(selected)}/members/${encodeURIComponent(userId)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ role: nextRole }) }); await loadFamily(selected); }
  async function remove(userId: string) { await request(`/api/v1/families/${encodeURIComponent(selected)}/members/${encodeURIComponent(userId)}`, { method: 'DELETE' }); await loadFamily(selected); }
  async function addAsset(event: FormEvent) { event.preventDefault(); await request(`/api/v1/families/${encodeURIComponent(selected)}/assets`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: assetName, category: assetCategory }) }); setAssetName(''); setNotice('家庭物品已保存。'); await loadFamily(selected); }
  async function saveBudget(event: FormEvent) { event.preventDefault(); await request(`/api/v1/families/${encodeURIComponent(selected)}/budget`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(budget) }); setNotice('共享预算已更新。'); await loadFamily(selected); }
  if (!localStorage.getItem('valuesee-token')) return <section className="page-section"><PageTitle icon={<Users size={22} />} title="我的家庭" subtitle="登录后可与家庭成员共享设备档案和购买提醒。" /><div className="panel"><Empty text="请先登录账户" /></div></section>;
  const editable = current?.role === 'owner' || current?.role === 'editor';
  return <section className="page-section"><PageTitle icon={<Users size={22} />} title="家庭空间" subtitle="共享家庭物品、消费预算与售后信息，写入权限由所有者控制。" />{notice && <div className="toast"><CheckCircle2 size={16} />{notice}</div>}{invitations.length > 0 && <section className="panel family-invitations"><h2>待确认邀请</h2>{invitations.map((item) => <article key={item.invitation_id}><div><strong>{item.family_name}</strong><span>角色 {item.role} · {date(item.expires_at)} 前有效</span></div><button onClick={() => void respond(item.invitation_id, true)}>接受</button><button className="danger" onClick={() => void respond(item.invitation_id, false)}>拒绝</button></article>)}</section>}<div className="family-workspace-grid"><aside className="panel"><h2>家庭</h2><form className="family-form" onSubmit={create}><input required value={name} onChange={(event) => setName(event.target.value)} /><button className="primary-button"><Plus size={15} />创建</button></form><div className="family-list">{families.map((item) => <button key={item.family_id} className={selected === item.family_id ? 'active' : ''} onClick={() => { setSelected(item.family_id); void loadFamily(item.family_id); }}><strong>{item.name}</strong><span>{item.role}</span></button>)}</div></aside><div className="family-workspace-main">{selected ? <><section className="panel"><div className="section-heading"><div><span>家庭档案</span><h2>共享物品</h2></div><b>{assets.length} 件</b></div>{editable && <form className="family-asset-form" onSubmit={addAsset}><input required placeholder="物品名称" value={assetName} onChange={(event) => setAssetName(event.target.value)} /><select value={assetCategory} onChange={(event) => setAssetCategory(event.target.value)}><option>数码设备</option><option>小家电</option><option>耗材</option><option>其他</option></select><button><Plus size={15} />添加</button></form>}<div className="family-assets">{assets.map((item) => <article key={item.asset_id}><div><strong>{item.name}</strong><span>{item.category}{item.brand ? ` · ${item.brand}` : ''}{item.model ? ` ${item.model}` : ''}</span></div><small>{item.warranty_deadline ? `保修至 ${date(item.warranty_deadline)}` : '未设置保修日期'}</small></article>)}{!assets.length && <Empty text="还没有家庭物品" />}</div></section><section className="panel"><h2>共享预算</h2><form className="family-budget-form" onSubmit={saveBudget}><label>每月预算<input type="number" min={0} disabled={!editable} value={budget.monthly_budget} onChange={(event) => setBudget({ ...budget, monthly_budget: Number(event.target.value) })} /></label><label>年度预算<input type="number" min={0} disabled={!editable} value={budget.annual_budget} onChange={(event) => setBudget({ ...budget, annual_budget: Number(event.target.value) })} /></label>{editable && <button className="primary-button"><Save size={15} />保存预算</button>}</form></section><section className="panel"><h2>成员与权限</h2>{current?.role === 'owner' && <form className="family-form" onSubmit={invite}><input type="email" required placeholder="已注册成员邮箱" value={inviteEmail} onChange={(event) => setInviteEmail(event.target.value)} /><button className="primary-button">发送邀请</button></form>}<div className="family-members">{members.map((item) => <article key={item.user_id}><div><strong>{item.display_name}</strong><span>{item.email} · {item.role}</span></div>{current?.role === 'owner' && item.role !== 'owner' && <div><button onClick={() => void role(item.user_id, item.role === 'editor' ? 'member' : 'editor')}>{item.role === 'editor' ? '设为只读成员' : '允许编辑'}</button><button className="danger" onClick={() => void remove(item.user_id)}>移除</button></div>}</article>)}</div></section></> : <div className="panel"><Empty text="创建或选择一个家庭" /></div>}</div></div></section>;
}
...
function PurchaseCenter({ purchases, products, purchaseProduct, paidPrice, onPurchaseProduct, onPaidPrice, onCreate, onRefresh }: { purchases: Purchase[]; products: Product[]; purchaseProduct: number; paidPrice: number; onPurchaseProduct: (value: number) => void; onPaidPrice: (value: number) => void; onCreate: (event: FormEvent) => void; onRefresh: () => Promise<void> }) {
  ...
  const [attachments, setAttachments] = useState<Record<string, Array<{ attachment_id: string; original_name: string; attachment_type: string }>>>({});
  const [notice, setNotice] = useState('');
  const loadTickets = async () => setTickets(await request<SupportTicket[]>('/api/v1/shopping/support/tickets'));
  useEffect(() => { void loadTickets().catch(() => undefined); }, []);
  async function loadAttachments(purchaseId: string) { const rows = await request<Array<{ attachment_id: string; original_name: string; attachment_type: string }>>(`/api/v1/shopping/purchases/${encodeURIComponent(purchaseId)}/attachments`); setAttachments((current) => ({ ...current, [purchaseId]: rows })); }
  async function upload(purchaseId: string, file: File) { const form = new FormData(); form.append('file', file); const type = file.type === 'application/pdf' ? 'invoice' : 'evidence'; await request(`/api/v1/shopping/purchases/${encodeURIComponent(purchaseId)}/attachments?attachment_type=${type}`, { method: 'POST', body: form }); await loadAttachments(purchaseId); setNotice('凭证已安全保存。'); }
  async function downloadAttachment(path: string, name: string) { const headers = new Headers(); const token = localStorage.getItem('valuesee-token'); if (token) headers.set('Authorization', `Bearer ${token}`); const response = await fetch(path, { headers }); if (!response.ok) throw new Error('附件下载失败'); const blob = await response.blob(); const url = URL.createObjectURL(blob); const anchor = document.createElement('a'); anchor.href = url; anchor.download = name; anchor.click(); URL.revokeObjectURL(url); }
  useEffect(() => { const handleAttachmentClick = (event: MouseEvent) => { const target = (event.target as HTMLElement).closest<HTMLAnchorElement>('a[href*="/api/v1/shopping/attachments/"]'); if (!target) return; event.preventDefault(); void downloadAttachment(target.getAttribute('href') || '', target.textContent?.trim() || 'attachment'); }; document.addEventListener('click', handleAttachmentClick); return () => document.removeEventListener('click', handleAttachmentClick); }, []);
  async function updateStatus(purchaseId: string, status: string) { await request(`/api/v1/shopping/purchases/${encodeURIComponent(purchaseId)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status }) }); await onRefresh(); }
  async function createTicket(purchase: Purchase) { const subject = window.prompt('问题标题，例如：商品降价需要保价帮助'); if (!subject?.trim()) return; const content = window.prompt('请描述问题和希望获得的帮助'); if (!content?.trim()) return; await request('/api/v1/shopping/support/tickets', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ purchase_id: purchase.purchase_id, category: 'after_sales', subject, content }) }); await loadTickets(); setNotice('客服工单已创建。'); }
  async function open(id: string) { setOpenTicket(await request<SupportTicket>(`/api/v1/shopping/support/tickets/${encodeURIComponent(id)}`)); }
  async function reply() { if (!openTicket) return; const content = window.prompt('回复客服'); if (!content?.trim()) return; const updated = await request<SupportTicket>(`/api/v1/shopping/support/tickets/${encodeURIComponent(openTicket.ticket_id)}/messages`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content }) }); setOpenTicket(updated); await loadTickets(); }
  return <section className="page-section"><PageTitle icon={<Receipt size={22} />} title="订单与售后" subtitle="从购买记录、凭证归档到保价和客服协同集中管理。" />{notice && <div className="toast"><CheckCircle2 size={16} />{notice}</div>}<div className="management-grid"><form className="panel compact-form" onSubmit={onCreate}><h3>记录一笔购买</h3><label>商品<select value={purchaseProduct} onChange={(e) => onPurchaseProduct(Number(e.target.value))}>{products.map((p, i) => <option key={i} value={i}>{p.title}</option>)}</select></label><label>实际支付<input type="number" min={0} value={paidPrice || products[purchaseProduct]?.price || 0} onChange={(e) => onPaidPrice(Number(e.target.value))} /></label><button className="primary-button" disabled={!products.length}><Receipt size={17} />保存购买</button></form><div className="panel list-panel"><h3>购买记录 <span>{purchases.length}</span></h3>{purchases.length ? purchases.map((item) => <article className="purchase-card" key={item.purchase_id}><div className="purchase-head"><strong>{item.product.title}</strong><b>{money(item.paid_price)}</b></div><span>{item.platform || '平台待确认'} · 购买于 {date(item.purchased_at)}</span><div className="deadline-row"><i>保价至 {date(item.price_protection_deadline)}</i><i>退货至 {date(item.return_deadline)}</i><i>保修至 {date(item.warranty_deadline)}</i></div><div className="purchase-actions"><select value={item.status} onChange={(e) => void updateStatus(item.purchase_id, e.target.value)}><option value="active">已购买</option><option value="received">已收货</option><option value="price_protection">保价中</option><option value="returning">退货中</option><option value="returned">已退货</option><option value="warranty">保修中</option><option value="completed">已完成</option></select><label className="soft-button"><Paperclip size={15} />上传凭证<input type="file" accept="application/pdf,image/jpeg,image/png,image/webp" onChange={(e) => e.target.files?.[0] && void upload(item.purchase_id, e.target.files[0])} /></label><button className="soft-button" onClick={() => { void loadAttachments(item.purchase_id); void createTicket(item); }}><LifeBuoy size={15} />联系客服</button></div>{attachments[item.purchase_id]?.length > 0 && <div className="attachment-list">{attachments[item.purchase_id].map((file) => <a key={file.attachment_id} href={`/api/v1/shopping/attachments/${file.attachment_id}/download`} target="_blank" rel="noreferrer"><Paperclip size={13} />{file.original_name}</a>)}</div>}</article>) : <Empty text="还没有购买记录" />}</div></div><section className="panel support-center"><div className="section-heading"><div><span>售后协同</span><h2>客服工单</h2></div><LifeBuoy size={22} /></div>{tickets.length ? tickets.map((ticket) => <button key={ticket.ticket_id} onClick={() => void open(ticket.ticket_id)}><div><strong>{ticket.subject}</strong><span>{ticket.category} · 更新于 {date(ticket.updated_at)}</span></div><b>{ticket.status}</b></button>) : <Empty text="还没有客服工单" />}</section>{openTicket && <div className="ticket-dialog panel"><button className="detail-close" title="关闭" onClick={() => setOpenTicket(null)}><Trash2 size={16} /></button><h2>{openTicket.subject}</h2><span>{openTicket.status}</span><div className="ticket-messages">{openTicket.messages?.map((message) => <article className={message.actor_role} key={message.message_id}><b>{message.actor_role === 'admin' ? 'ValuSee 客服' : '我'}</b><p>{message.content}</p><small>{date(message.created_at)}</small></article>)}</div><button className="primary-button" onClick={() => void reply()}>继续回复</button></div>}</section>;
}
