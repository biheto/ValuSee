...
- Browser extension v0.4.3 adds JD-specific price/SKU prioritization and navigation-noise filtering. Public URL parsing now rejects short platform-shell titles such as `pind` and only auto-adds a link when the server reports a parsed, meaningful product; blocked or partial pages remain in the recovery flow for extension/OCR/manual confirmation.
...
| Screenshot OCR | Productized | Secure upload, SHA-256, MIME/size validation, on-device Tesseract.js Chinese/English OCR, optional multimodal enhancement, server Tesseract fallback, confidence/missing-field reporting, and explicit failure states are implemented. OCR assets are self-hosted and upload filenames are never used as recognized product titles. |
...
| Browser extension | Productized | Downloadable Manifest V3 package connects to the user's account, uses platform-aware title/price/SKU/variant extraction, provides editable visible-page capture, and writes a final-confirmation inbox. |
...
| Pinduoduo official search | Implemented, credential-gated | Direct signed `pdd.ddk.goods.search` integration, product normalization, optional PID promotion links, health checks, source evidence, homepage results and comparison import. Live data remains disabled until production credentials are configured. |
